...
namespace NXmpp.NCommon {
	...
	internal class Guard {
		...
		internal static void Against<TException>(bool assertion, string message) where TException : Exception {
			if (assertion)
		...
		internal static void Against<TException>(Func<bool> assertion, string message) where TException : Exception {
		...
		/// <param name="message">string. The exception message to throw.</param>
		internal static void InheritsFrom<TBase>(object instance, string message) where TBase : Type {
		...
		/// <param name="message">string. The exception message to throw.</param>
		internal static void InheritsFrom<TBase>(Type type, string message) {
		...
		/// <param name="message">string. The exception message to throw.</param>
		internal static void Implements<TInterface>(object instance, string message) {
		...
		/// <param name="message">string. The exception message to throw.</param>
		internal static void Implements<TInterface>(Type type, string message) {
		...
		internal static void TypeOf<TType>(object instance, string message) {
		...
		internal static void IsEqual<TException>(object compare, object instance, string message) where TException : Exception {
			if (compare != instance)
				throw (TException)Activator.CreateInstance(typeof(TException), message);
		}

		internal static void AgainstArgumentNullException<T>(Expression<Func<T>> parameter) where T : class {
			string parameterName = ((MemberExpression)parameter.Body).Member.Name;
			Against<ArgumentNullException>(parameter.Compile()() == null, parameterName + " cannot be null");
		}
	}
}